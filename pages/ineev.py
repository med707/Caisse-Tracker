...
if data:
    df = pd.DataFrame(data, columns=["Date", _("categorie"), _("sous_categorie"), "Total"])
    ...
    category_totals = df.groupby([_("categorie"), "Date"])["Total"].sum().reset_index()
    ...
    date_totals = df.groupby("Date")["Total"].sum().reset_index().sort_values("Date", ascending=True)
    ...
    for i, date in enumerate(dates):
        ...
        total = date_totals[date_totals["Date"] == date_str]["Total"].sum()
    ...
    if not category_totals.empty:
        ...
        stacked_df = category_totals.pivot(index=_("categorie"), columns="Date", values="Total").fillna(0)
